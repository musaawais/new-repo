...
// ── Inline local-proxy tunnel ─────────────────────────────────────────────────
// Spins up a tiny Node.js HTTP proxy on 127.0.0.1 per visit.
// Electron talks to localhost (no auth needed). The local server injects
// Proxy-Authorization into every CONNECT request to the upstream proxy.
// This bypasses Chromium's auth system entirely — residential proxies that
// drop the connection instead of returning 407 are handled correctly.

interface LocalProxy { port: number; close: () => void; }
interface UpstreamProxy { host: string; port: number; username?: string; password?: string; }

function startLocalProxy(upstream: UpstreamProxy): Promise<LocalProxy> {
  ...
  return new Promise((resolve, reject) => {
    ...
    const server = http.createServer((clientReq, clientRes) => {
      ...
      if (authHeader) (options.headers as Record<string,string>)['Proxy-Authorization'] = authHeader;
      const pr = http.request(options, (res) => {
        clientRes.writeHead(res.statusCode ?? 200, res.headers);
        res.pipe(clientRes, { end: true });
      });
      pr.on('error', () => clientRes.end());
      clientReq.pipe(pr, { end: true });
    });

    server.on('connect', (req, clientSocket: import('net').Socket, head: Buffer) => {
      // HTTPS CONNECT tunnel — open raw TCP to upstream and inject auth
      const upSock = net.connect(upstream.port, upstream.host);
      upSock.on('error', () => { try { clientSocket.end('HTTP/1.1 502 Bad Gateway\r\n\r\n'); } catch { /* ignore */ } });
      upSock.on('connect', () => {
        // Build the CONNECT request as a plain ASCII string — safe because
        // CONNECT headers are always ASCII (host:port + base64 auth).
        const connectReq = [
        ...
        upSock.write(connectReq);

        // Accumulate response bytes as Buffers — NEVER decode as UTF-8.
        // After the 200 response header, the stream carries raw TLS bytes;
        // string conversion corrupts them.
        let headerBuf = Buffer.alloc(0);
        const onData = (chunk: Buffer) => {
          headerBuf = Buffer.concat([headerBuf, chunk]);
          const sep = headerBuf.indexOf(Buffer.from('\r\n\r\n'));
          if (sep === -1) return;  // haven't received full response header yet

          upSock.removeListener('data', onData);
          const headerStr = headerBuf.slice(0, sep).toString('ascii');
          const statusCode = parseInt((headerStr.split('\r\n')[0].split(' ')[1] ?? '0'), 10);

          if (statusCode >= 200 && statusCode < 300) {
            // Tell Electron the tunnel is open
            clientSocket.write('HTTP/1.1 200 Connection Established\r\n\r\n');
            // Any bytes after the header separator belong to the TLS handshake
            ...
            // Bidirectional pipe — raw bytes from here on
        ...
        upSock.on('error', () => { try { clientSocket.destroy(); } catch { /* ignore */ } });
        clientSocket.on('error', () => { try { upSock.destroy(); } catch { /* ignore */ } });
      });
    });
    ...
    server.listen(0, '127.0.0.1', () => {
      ...
      resolve({ port, close: () => { try { (server as any).closeAllConnections?.(); server.close(); } catch { /* ignore */ } } });
    });
  });
}
